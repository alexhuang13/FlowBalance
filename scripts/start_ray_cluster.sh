...
# Examples:
#   AUTO_TRAIN=1 TRAIN_SCRIPT=recipe/flowopsd/run_math_flowopsd.sh ... bash scripts/start_ray_cluster.sh
#   TRAIN_SCRIPT=run_math_flowopsd.sh ... bash scripts/start_ray_cluster.sh
...
resolve_train_script() {
  ...
  for candidate in \
    "${REPO_ROOT}/recipe/flowopsd/${script}" \
