...
"""FSDP worker wrapper for FlowSD."""
...
from recipe.flowsd.flowsd_dp_actor import FlowSDDataParallelPPOActor, TrustRegionTeacher
from recipe.sdpo.sdpo_fsdp_workers import SDPOAsyncActorRolloutRefWorker

logger = logging.getLogger(__file__)
logger.setLevel(os.getenv("VERL_LOGGING_LEVEL", "WARN"))

__all__ = ["FlowSDAsyncActorRolloutRefWorker"]


class FlowSDAsyncActorRolloutRefWorker(SDPOAsyncActorRolloutRefWorker):
    """Actor-rollout-ref worker with frozen-reference FlowSD log-prob APIs."""
    ...
    def init_model(self):
        ...
        self.actor = FlowSDDataParallelPPOActor(
            config=actor_cfg,
        ...
        if self.rank == 0:
            logger.warning("[FlowSD] actor wrapped; loss_mode=%s", loss_mode)
    ...
    def _compute_ref_module_log_prob(
        ...
        log_probs = []
        # FlowSD evaluates both privileged and original contexts with the same
        # frozen initial reference module. ``module`` remains overridable for the
        # inherited SDPO path, but FlowSD never updates the reference weights.
    ...
    def compute_flowsd_ref_log_probs(self, data: DataProto) -> DataProto:
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    def compute_flowsd_log_probs(self, data: DataProto) -> DataProto:
