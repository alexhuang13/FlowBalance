...
class FlowSDTaskRunner(CustomTaskRunner):
    """TaskRunner that wires FlowSD worker and trainer into FlowSD."""

    @staticmethod
    def _uses_ref(config) -> bool:
        loss_mode = config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla")
        return loss_mode in {"sdpo", "flowsd"}

    def add_actor_rollout_worker(self, config):
        from verl.single_controller.ray import RayWorkerGroup
        from verl.trainer.ppo.ray_trainer import Role

        from recipe.flowsd.flowsd_fsdp_workers import FlowSDAsyncActorRolloutRefWorker

        strategy = config.actor_rollout_ref.actor.strategy
        if strategy not in {"fsdp", "fsdp2"}:
            raise NotImplementedError(f"FlowSD only supports fsdp/fsdp2, got strategy={strategy}")
        role = Role.ActorRolloutRef if self._uses_ref(config) else Role.ActorRollout
        self.role_worker_mapping[role] = ray.remote(FlowSDAsyncActorRolloutRefWorker)
        self.mapping[role] = "global_pool"
        return FlowSDAsyncActorRolloutRefWorker, RayWorkerGroup
    ...
    def run(self, config):
        ...
        from recipe.flowsd.flowsd_ray_trainer import FlowSDRayPPOTrainer

        core_main.CustomRayPPOTrainer = FlowSDRayPPOTrainer
        return super().run(config)


@hydra.main(config_path="config", config_name="flowsd_trainer", version_base=None)
def main(config):
    auto_set_device(config)
    run_ppo(config, task_runner_class=ray.remote(num_cpus=1)(FlowSDTaskRunner))
