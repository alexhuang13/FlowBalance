...
#
# Training/model/data/optimizer/rollout/reward/FlowSD settings are pinned to
# the effective Run28 values. Across runs this script changes only:
#   1. FLOWSD_ETA_R (the ablation variable), and
...
ETA_SWEEP_EXP_PREFIX=${ETA_SWEEP_EXP_PREFIX:-FlowSD-Qwen3-8B-math-dapo17k}
...
echo "[FlowSD eta sweep] strict Run28 ablation; eta_R values: ${etas[*]}"
echo "[FlowSD eta sweep] fixed: beta_q=1, LR=1e-6, 4x8 H20 layout, SP=8, batch=256, n=8, lengths=2048/8192/10240"
echo "[FlowSD eta sweep] each run trains through step ${VAL_STEP}, then completes the same benchmark validation"
...
for index in "${!etas[@]}"; do
    ...
    echo "[FlowSD eta sweep] [$((index + 1))/${#etas[@]}] eta_R=${eta}, experiment=${exp_name}"
    if [[ -f "${complete_file}" ]]; then
        echo "[FlowSD eta sweep] already complete, skipping: ${complete_file}"
        continue
    fi
    ...
    FLOWSD_BETA_Q=1 \
    FLOWSD_ETA_R="${eta}" \
    FLOWSD_BETA_Q_START=null \
    FLOWSD_BETA_Q_END=null \
    FLOWSD_ETA_R_START=null \
    FLOWSD_ETA_R_END=null \
    FLOWSD_SCHEDULE_STEPS=0 \
    FLOWSD_FLOW_GAP_CLIP_LOW=5.0 \
    FLOWSD_FLOW_GAP_CLIP_HIGH=5.0 \
    FLOWSD_IMPORTANCE_RATIO_CAP=1.2 \
    FLOWSD_RESIDUAL_LENGTH_SCALE=0.0 \
    FLOWSD_CLIP_B=4.0 \
    FLOWSD_RHO=1.0 \
    FLOWSD_LAMBDA_KL=0.0 \
    FLOWSD_USE_KL=False \
    FLOWSD_GATE_NO_CONTEXT=drop \
    FLOWSD_MIN_GROUP_VALID=2 \
    FLOWSD_W_CLIP=null \
    FLOWSD_W_MIN=0.0 \
    FLOWSD_W_MAX=10.0 \
    ...
    bash "${SCRIPT_DIR}/run_math_flowsd.sh"

    echo "[FlowSD eta sweep] submitted eta_R=${eta}; waiting for training + benchmark completion"
    ...
    while [[ ! -f "${complete_file}" ]]; do
        ...
        echo "[FlowSD eta sweep] eta_R=${eta}: ${state}; waiting ${ETA_SWEEP_POLL_SECONDS}s"
        sleep "${ETA_SWEEP_POLL_SECONDS}"
    done
    echo "[FlowSD eta sweep] eta_R=${eta} complete: ${complete_file}"
done

echo "[FlowSD eta sweep] all strict Run28 eta_R arms completed: ${etas[*]}"
