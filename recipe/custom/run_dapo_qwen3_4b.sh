...
ray job submit --address="${RAY_ADDRESS}" --no-wait --runtime-env="${RUNTIME_ENV}" \
    ...
    reward_model.api_endpoints='[{name: "ifeval:flan:ifeval_old:code:code_stdio", endpoint: "http://28.7.194.238:1217/execute"}, {name: "general-quality_ref:general-quality", endpoint: "http://28.7.194.238:8217/generate"}]' \
