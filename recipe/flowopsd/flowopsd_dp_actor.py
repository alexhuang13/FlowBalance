...
__all__ = ["FlowOPSDDataParallelPPOActor", "TrustRegionTeacher"]
...
class FlowOPSDDataParallelPPOActor(SDPODataParallelPPOActor):
    """SDPO-compatible actor with a length-normalized sequence-level FlowOPSD branch."""

    @GPUMemoryLogger(role="flowopsd dp actor", logger=logger)
    def update_policy(self, data: DataProto):
        loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
        if loss_mode != "flowopsd":
            return super().update_policy(data)
        return self._update_policy_flowopsd(data)

    def _update_policy_flowopsd(self, data: DataProto):
        self.actor_module.train()
        temperature = data.meta_info["temperature"]
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        flow_cfg = getattr(self.config, "flow_opsd", None)
        if flow_cfg is None:
            raise ValueError("loss_mode=flowopsd requires actor.flow_opsd config.")
        ...
        assert not missing, f"Missing required FlowOPSD keys: {missing}"
        ...
        if _cfg_get(flow_cfg, "use_flowopsd_kl", False) or _cfg_get(flow_cfg, "lambda_kl", 0.0) > 0:
            if "ref_log_prob" not in data.batch.keys():
                raise ValueError("FlowOPSD KL requires ref_log_prob in batch.")
        ...
        for _ in range(self.config.ppo_epochs):
            for mini_batch in mini_batches:
                local_valid = mini_batch.batch["flowopsd_mask"].float().sum()
                mini_valid = local_valid.clamp(min=1.0)

                # FSDP collectives must be entered in exactly the same order on every
                # rank. A FlowOPSD shard can have no valid privileged-context samples
                ...
                if not global_has_valid:
                    ...
                    _append_to_metrics(
                        metrics,
                        {
                            "flowopsd/global_empty_mini_batch": 1.0,
                ...
                for micro_batch in micro_batches:
                    ...
                    flowopsd_mask = model_inputs["flowopsd_mask"].float()
                    flowopsd_target = model_inputs["flowopsd_target"].float()
                    ...
                    residual = seq_logp - flowopsd_target
                    ...
                    pg_loss = (tb_per_sample * flowopsd_mask).sum() / mini_valid.to(tb_per_sample.device)
                    policy_loss = pg_loss
                    micro_metrics = {
                        "flowopsd/tb_loss": pg_loss.detach().item(),
                        "flowopsd/residual_abs_mean": (
                            residual.detach().abs()[flowopsd_mask > 0.5].mean().item()
                            if (flowopsd_mask > 0.5).any()
                            else 0.0
                        ),
                        "flowopsd/seq_logp_student_mean": (
                            seq_logp.detach()[flowopsd_mask > 0.5].mean().item()
                            if (flowopsd_mask > 0.5).any()
                            else 0.0
                        ),
                        "flowopsd/seq_logp_student_raw_mean": (
                            seq_logp_raw.detach()[flowopsd_mask > 0.5].mean().item()
                            if (flowopsd_mask > 0.5).any()
                            else 0.0
                        ),
                        "flowopsd/target_actor_mean": (
                            flowopsd_target.detach()[flowopsd_mask > 0.5].mean().item()
                            if (flowopsd_mask > 0.5).any()
                            else 0.0
                        ),
                        "flowopsd/micro_valid": flowopsd_mask.sum().item(),
                    }
                    if raw_weight.numel() > 0:
                        denom = weights.pow(2).sum().clamp(min=1e-8)
                        micro_metrics["flowopsd/nESS"] = (weights.sum().pow(2) / (weights.numel() * denom)).item()
                        micro_metrics["flowopsd/is_weight_raw_mean"] = raw_weight.mean().item()
                        micro_metrics["flowopsd/is_weight_raw_max"] = raw_weight.max().item()
                        micro_metrics["flowopsd/is_weight_mean"] = weights.mean().item()
                        micro_metrics["flowopsd/is_weight_max"] = weights.max().item()

                    lambda_kl = float(_cfg_get(flow_cfg, "lambda_kl", 0.0))
                    if _cfg_get(flow_cfg, "use_flowopsd_kl", False) and lambda_kl > 0:
                        ref_log_prob = model_inputs["ref_log_prob"]
                        kld = kl_penalty(logprob=log_prob, ref_logprob=ref_log_prob, kl_penalty=self.config.kl_loss_type)
                        kl_loss = agg_loss(loss_mat=kld, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
                        policy_loss = policy_loss + lambda_kl * kl_loss
                        micro_metrics["flowopsd/kl_loss"] = kl_loss.detach().item()
