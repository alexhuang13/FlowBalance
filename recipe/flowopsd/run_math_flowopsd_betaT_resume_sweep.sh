...
set -Eeuo pipefail
trap 'rc=$?; echo "[FlowOPSD beta_T sweep] failed at line ${LINENO}: ${BASH_COMMAND} (exit=${rc})" >&2; exit ${rc}' ERR
...
SOURCE_RUN=${SOURCE_RUN:-${OUTPUT_ROOT}/verl-flowopsd/FlowOPSD-Qwen3-8B-math-dapo17k-betaq1-grpo-signadv-frozenref-etaR15-lr1e6-run28}
...
echo "[FlowOPSD beta_T sweep] source checkpoint: $SOURCE_CHECKPOINT"
echo "[FlowOPSD beta_T sweep] beta_T maps to code field flow_opsd.beta_q"
echo "[FlowOPSD beta_T sweep] values: ${betas[*]}; serial training + complete benchmark evaluation"

for index in "${!betas[@]}"; do
  beta=${betas[$index]}; beta_tag=${beta//./p}
  exp_name="FlowOPSD-Qwen3-8B-math-dapo17k-betaT${beta_tag}-etaR15-lr1e6-${RUN_TAG}"
  run_dir="${OUTPUT_ROOT}/${PROJECT_NAME}/${exp_name}"
  complete_file="${run_dir}/val/step_${VAL_STEP}_math_benchmarks_5seeds/.complete"

  echo "[FlowOPSD beta_T sweep] [$((index+1))/${#betas[@]}] beta_T=${beta}; experiment=${exp_name}"
  if [[ -f "$complete_file" ]]; then
    echo "[FlowOPSD beta_T sweep] already complete: $complete_file"
    continue
  fi
  ...
  args=(
    ...
    FLOWOPSD_BETA_Q="$beta" FLOWOPSD_ETA_R=15 FLOWOPSD_BETA_Q_START=null FLOWOPSD_BETA_Q_END=null
    FLOWOPSD_ETA_R_START=null FLOWOPSD_ETA_R_END=null FLOWOPSD_SCHEDULE_STEPS=0
    FLOWOPSD_FLOW_GAP_CLIP_LOW=5.0 FLOWOPSD_FLOW_GAP_CLIP_HIGH=5.0 FLOWOPSD_IMPORTANCE_RATIO_CAP=1.2
    FLOWOPSD_RESIDUAL_LENGTH_SCALE=0.0 FLOWOPSD_CLIP_B=4.0 FLOWOPSD_RHO=1.0 FLOWOPSD_LAMBDA_KL=0.0
    FLOWOPSD_USE_KL=False FLOWOPSD_GATE_NO_CONTEXT=drop FLOWOPSD_MIN_GROUP_VALID=2
    FLOWOPSD_W_CLIP=null FLOWOPSD_W_MIN=0.0 FLOWOPSD_W_MAX=10.0
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    printf '[FlowOPSD beta_T sweep] command:'; printf ' %q' env "${args[@]}" bash "${SCRIPT_DIR}/run_math_flowopsd.sh"; printf '\n'
    continue
  fi

  env "${args[@]}" bash "${SCRIPT_DIR}/run_math_flowopsd.sh"
  echo "[FlowOPSD beta_T sweep] submitted beta_T=${beta}; waiting for step-180 training and all benchmark units"
  ...
  while [[ ! -f "$complete_file" ]]; do
    ...
    echo "[FlowOPSD beta_T sweep] beta_T=${beta}: ${state}; sleeping ${POLL_SECONDS}s"
    sleep "$POLL_SECONDS"
  done
  echo "[FlowOPSD beta_T sweep] beta_T=${beta} complete: $complete_file"
done

echo "[FlowOPSD beta_T sweep] all arms complete: ${betas[*]}"
