...
from recipe.flowopsd.flowopsd_core_algos import compute_flowopsd_target, resolve_flowopsd_coefficients
from recipe.sdpo.sdpo_ray_trainer import SDPORayPPOTrainer

__all__ = ["FlowOPSDRayPPOTrainer"]


class FlowOPSDRayPPOTrainer(SDPORayPPOTrainer):
    """Adds FlowOPSD target construction while preserving SDPO and vanilla paths."""
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores):
        ...
        print(f"[FlowOPSD validation] step={self.global_steps}, showing {len(samples)} samples")
        for idx, (sample_input, sample_output, sample_score) in enumerate(samples, start=1):
            print(f"[FlowOPSD validation][{idx}] score={sample_score}")
            print(f"[FlowOPSD validation][{idx}] input:\n{shorten(sample_input, 2000)}")
            print(f"[FlowOPSD validation][{idx}] output:\n{shorten(sample_output, 4000)}")
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        flow_cfg = self.config.actor_rollout_ref.actor.get("flow_opsd", None)
        beta_q, _, _, _ = resolve_flowopsd_coefficients(flow_cfg, int(self.global_steps))
        ...
        if reward_type == "grpo_advantage":
            if "advantages" not in batch.batch:
                raise ValueError("flow_opsd.reward_type=grpo_advantage requires batch advantages")
        ...
        if beta_zero:
            ...
            flow_log_probs = self.actor_rollout_wg.compute_flowopsd_ref_log_probs(batch)
            batch = batch.union(flow_log_probs)
            teacher_log_prob = None
            self_distillation_mask = None
            group_keys = batch.non_tensor_batch["uid"]
            flow_metrics["flowopsd/skipped_privileged_teacher"] = 1.0
        else:
            sd_cfg = self.config.actor_rollout_ref.actor.get("self_distillation", None)
            if sd_cfg is None:
                raise ValueError("loss_mode=flowopsd with beta_q>0 requires actor.self_distillation config")
            raw_reward = batch.batch["token_level_scores"]
            reward_extra_infos_dict = self._reconstruct_reward_extra_infos(batch, sd_cfg)
            sd_data = self._maybe_build_flowopsd_reprompt_batch(batch, raw_reward, reward_extra_infos_dict)
            if sd_data is not None:
                sd_batch, sd_metrics = sd_data
                batch = batch.union(sd_batch)
                flow_metrics.update(sd_metrics)
            flow_log_probs = self.actor_rollout_wg.compute_flowopsd_log_probs(batch)
            batch = batch.union(flow_log_probs)
            teacher_log_prob = batch.batch["teacher_log_prob"]
            self_distillation_mask = batch.batch["self_distillation_mask"]
            group_keys = batch.non_tensor_batch.get("flowopsd_group_key", batch.non_tensor_batch["uid"])
            flow_metrics["flowopsd/skipped_privileged_teacher"] = 0.0

        target, mask, target_metrics = compute_flowopsd_target(
            ...
            uids=group_keys,
            flow_opsd_config=flow_cfg,
            global_step=int(self.global_steps),
        )
        batch = batch.union(DataProto.from_dict(tensors={"flowopsd_target": target, "flowopsd_mask": mask}))
    ...
    def _maybe_build_flowopsd_reprompt_batch(
        self,
        batch: DataProto,
        ...
        return (
            DataProto.from_dict(
                ...
                non_tensors={"flowopsd_group_key": group_keys},
            ),
            metrics,
        )
