#!/usr/bin/env bash
# FlowOPSD math run script. Defaults reproduce the Run28 training setup;
# FLOWOPSD_ETA_R is the intended ablation knob.
set -xeuo pipefail

project_name=${PROJECT_NAME:-verl-flowopsd}
exp_name=${EXP_NAME:-FlowOPSD-Qwen3-8B-math-dapo17k-betaq1-grpo-signadv-frozenref-etaR15-lr1e6-run28-repro}
...
beta_q=${FLOWOPSD_BETA_Q:-1}
eta_R=${FLOWOPSD_ETA_R:-15}
beta_q_start=${FLOWOPSD_BETA_Q_START:-null}
beta_q_end=${FLOWOPSD_BETA_Q_END:-null}
eta_R_start=${FLOWOPSD_ETA_R_START:-null}
eta_R_end=${FLOWOPSD_ETA_R_END:-null}
flow_schedule_steps=${FLOWOPSD_SCHEDULE_STEPS:-0}
flow_gap_clip_low=${FLOWOPSD_FLOW_GAP_CLIP_LOW:-5.0}
flow_gap_clip_high=${FLOWOPSD_FLOW_GAP_CLIP_HIGH:-5.0}
importance_ratio_cap=${FLOWOPSD_IMPORTANCE_RATIO_CAP:-1.2}
residual_length_scale=${FLOWOPSD_RESIDUAL_LENGTH_SCALE:-0.0}
...
clip_B=${FLOWOPSD_CLIP_B:-4.0}
rho=${FLOWOPSD_RHO:-1.0}
lambda_kl=${FLOWOPSD_LAMBDA_KL:-0.0}
use_flowopsd_kl=${FLOWOPSD_USE_KL:-False}
gate_no_context=${FLOWOPSD_GATE_NO_CONTEXT:-drop}
min_group_valid=${FLOWOPSD_MIN_GROUP_VALID:-2}
w_clip=${FLOWOPSD_W_CLIP:-null}
w_min=${FLOWOPSD_W_MIN:-0.0}
w_max=${FLOWOPSD_W_MAX:-10.0}
...
ray job submit --address="${RAY_ADDRESS}" --no-wait --runtime-env="${RUNTIME_ENV}" \
    -- python3 -m recipe.flowopsd.main_flowopsd \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=flowopsd \
    ...
    actor_rollout_ref.actor.flow_opsd.beta_q=${beta_q} \
    actor_rollout_ref.actor.flow_opsd.eta_R=${eta_R} \
    actor_rollout_ref.actor.flow_opsd.reward_type=grpo_advantage \
    actor_rollout_ref.actor.flow_opsd.beta_q_start=${beta_q_start} \
    actor_rollout_ref.actor.flow_opsd.beta_q_end=${beta_q_end} \
    actor_rollout_ref.actor.flow_opsd.eta_R_start=${eta_R_start} \
    actor_rollout_ref.actor.flow_opsd.eta_R_end=${eta_R_end} \
    actor_rollout_ref.actor.flow_opsd.schedule_steps=${flow_schedule_steps} \
    actor_rollout_ref.actor.flow_opsd.flow_gap_clip_low=${flow_gap_clip_low} \
    actor_rollout_ref.actor.flow_opsd.flow_gap_clip_high=${flow_gap_clip_high} \
    actor_rollout_ref.actor.flow_opsd.importance_ratio_cap=${importance_ratio_cap} \
    actor_rollout_ref.actor.flow_opsd.residual_length_scale=${residual_length_scale} \
    actor_rollout_ref.actor.flow_opsd.clip_B=${clip_B} \
    actor_rollout_ref.actor.flow_opsd.rho=${rho} \
    actor_rollout_ref.actor.flow_opsd.lambda_kl=${lambda_kl} \
    actor_rollout_ref.actor.flow_opsd.use_flowopsd_kl=${use_flowopsd_kl} \
    actor_rollout_ref.actor.flow_opsd.gate_no_context=${gate_no_context} \
    actor_rollout_ref.actor.flow_opsd.min_group_valid=${min_group_valid} \
    actor_rollout_ref.actor.flow_opsd.w_clip=${w_clip} \
    actor_rollout_ref.actor.flow_opsd.w_min=${w_min} \
    actor_rollout_ref.actor.flow_opsd.w_max=${w_max} \
...
if [[ "${step180_val_enable}" == "1" ]]; then
    echo "Submitting FlowOPSD step-${step180_val_step} benchmark validation watcher"
    RUN_DIR="${CKPTS_DIR}" \
