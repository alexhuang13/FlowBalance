...
def test_beta_zero_uses_all_rollouts_and_grpo_advantage():
    ...
    target, mask, metrics = compute_flowopsd_target(
        ...
        flow_opsd_config=_cfg(),
    )

    # beta_q=0 ignores privileged-context availability and min_group_valid=8.
    torch.testing.assert_close(mask, torch.ones(4))
    assert metrics["flowopsd/beta_zero_flowrl_path"] == 1.0
    assert metrics["flowopsd/reward_is_grpo_advantage"] == 1.0
...
def test_beta_zero_schedule_is_resolved_for_fast_path():
    ...
    beta_q, eta_R, beta_progress, _ = resolve_flowopsd_coefficients(cfg, global_step=0)
    assert beta_q == 0.0
    assert eta_R == 15.0
...
def test_positive_beta_keeps_privileged_gate():
    target, mask, metrics = compute_flowopsd_target(
        ...
        flow_opsd_config=_cfg(beta_q=1.0, min_group_valid=1),
    )
    del target
    torch.testing.assert_close(mask, torch.tensor([1.0, 0.0]))
    assert metrics["flowopsd/beta_zero_flowrl_path"] == 0.0
...
def test_positive_beta_multiplies_gain_by_grpo_advantage_sign():
    ...
    _, _, metrics = compute_flowopsd_target(
    ...
    assert metrics["flowopsd/G_q_raw_mean"] == 2.0
    assert metrics["flowopsd/G_q_mean"] == 0.0
    assert metrics["flowopsd/advantage_zero_fraction"] == 1.0 / 3.0
