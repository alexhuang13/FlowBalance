...
class CustomSFTDataset(SFTDataset):
    ...
    def __init__(self, parquet_files: str | ListConfig, tokenizer, config, max_samples: int = -1):
        super().__init__(parquet_files=parquet_files, tokenizer=tokenizer, config=config, max_samples=max_samples)
    ...
    def __getitem__(self, item):
        ...
        # apply chat template if str
        prompt_chat = [{"role": "user", "content": prompt}] if type(prompt) is str else prompt
