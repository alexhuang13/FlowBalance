#!/usr/bin/env python3
"""Evaluate SDPO rerun checkpoint step 180 with five seeds.
...
OUT = WORK / 'results_sdpo_rerun_step180_5seeds_t06_p095_k20_len38912_b16'
MERGED = WORK / 'merged_models/sdpo_rerun_step180_5seeds'
SEEDS = tuple(range(5))
NEW_SEEDS = SEEDS
ALL = ['aime24', 'aime25', 'aime26', 'hmmt25', 'minerva_math', 'math500', 'olympiadbench']
AIME = ['aime24', 'aime25', 'aime26']
RUNS = {
    'sdpo': ROOT / 'output/verl-sdpo/SDPO-Qwen3-8B-math-strict-dapo17k-rerun-keep-step150-180',
}
RAW_FIELDS = [
...
def collect_rows():
    ...
    for p in OUT.glob('seed_*/*/*/*/summary_s0_e-1_n*_seed*.json'):
        ...
        try:
            ...
            continue
        if algorithm not in RUNS or step != 180 or seed not in SEEDS:
            continue
...
def formatted_table(stats, setting, metric, datasets):
    ...
    order = [a.upper() + '-' + str(s) for a in RUNS for s in (180,)]
...
def aggregate():
    ...
    md = [
        '# SDPO Rerun Step 180 — Five-Seed Results', '',
...
def main():
    ...
    print('[done] SDPO step180 five-seed experiment', flush=True)
