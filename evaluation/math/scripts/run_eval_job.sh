...
FLOWOPSD_RUN=${ROOT}/output/verl-flowopsd/FlowOPSD-Qwen3-8B-math-dapo17k-betaq1-grpo-signadv-frozenref-etaR15-lr1e6-run28
...
discover_checkpoints() {
 ...
 for pair in "grpo:${GRPO_RUN}" "rlsd:${RLSD_RUN}" "flowopsd:${FLOWOPSD_RUN}"; do
...
for p in root.glob('*/*/summary_s0_e-1_n16_seed0.json'):
 ...
 m=re.fullmatch(r'(grpo|rlsd|flowopsd)_step_(\d+)',d.get('model_tag',''))
 if not m: continue
